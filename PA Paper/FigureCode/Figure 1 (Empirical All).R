...
postscript(paper="special", width=6.5, height=4, file="~/Dropbox/Dimensionality/PAFinal/PaperFiles/TexFiles/EmpAll.ps",  pagecentre=TRUE, onefile=FALSE, horizontal=FALSE)
NOMRed <- subset(NOM, NOM$chamber=="sen" & NOM$whichSubset=="All")
...
predd <- predict(thisMod, thisSeq, se=TRUE)
lines(thisSeq, predd$fit, col="gray60", lwd=2, lty=4)
legend("topright", c(paste("APRE2", "-", "APRE1"), "APRE3 - APRE2", "APRE3 - APRE4"),  lwd=1, col=c("gray10", "gray40", "gray60"), cex=.8, lty=c(1,2,4), pch=c(19, 17, 7), bty="n")
dev.off()
