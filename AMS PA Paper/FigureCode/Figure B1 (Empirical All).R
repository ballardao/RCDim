...
## Figure 1: Empirical APRE
postscript(paper="special", width=6.5, height=4, file="~/Dropbox/Dimensionality/PAFinal/PaperFiles/TexFiles/B1EmpAll.ps",  pagecentre=TRUE, onefile=FALSE, horizontal=FALSE)
NOMRed <- subset(NOM, NOM$chamber=="sen" & NOM$whichSubset%in%c("Dem Votes", "Rep Votes"))
...
legend("topleft", c("APRE1", "APRE2", "APRE3"), lwd=2, col=c("gray10", "gray40", "gray60"), lty=c(1,2,4), pch=c(19,17,7),cex=.8, bty="n", )
par(xaxt="s")
...
predd <- predict(thisMod, thisSeq, se=TRUE)
lines(thisSeq, predd$fit, col="gray60", lwd=2, lty=4)
legend("topright", c("APRE2-APRE1", "APRE3-APRE2", "APRE3-APRE4"),  lwd=2, col=c("gray10", "gray40", "gray60"), cex=.8, lty=c(1,2,4), pch=c(19, 17, 7), bty="n")
dev.off()
