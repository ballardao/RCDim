...
postscript(paper="special", width=7, height=7, file="~/Dropbox/Dimensionality/PAFinal/PaperFiles/TexFiles/Figure5.ps",  pagecentre=TRUE, onefile=FALSE, horizontal=FALSE)
psValues <- names(table(results$partySeparation))
par(mfrow=c(2,4))
par(mgp=c(1,0,0), mar=c(3,2,1.5,1), tcl=.05)
temp <- c(.5, 1, 1.5)
this.data1 <- subset(results,  partySeparation%in%psValues[1:15]  & nSeparateDimensions%in%1:4 & label%in%c(2,3) & nDimensions%in%c(1) & radius%in%c(7:11) & beta%in%temp)
this.data2 <- subset(results,  partySeparation%in%psValues[1:15]  & nSeparateDimensions%in%1:4 & label%in%c(2,3) & nDimensions%in%c(7)& radius%in%c(7:11) & beta%in%temp)
this.data3 <- subset(results,  partySeparation%in%psValues[1:15]  & nSeparateDimensions%in%1:4 & label%in%c(2,3) & nDimensions%in%c(15)& radius%in%c(7:11)& beta%in%temp)
this.data4 <- subset(results,  partySeparation%in%psValues[1:15]  & nSeparateDimensions%in%1:4 & label%in%c(2,3) & nDimensions%in%c(4)& radius%in%c(7:11)& beta%in%temp)
...
with(this.data4, sub.plotter.diff(apreD1, apreD2, apreD3, apreD4, partySeparation, .ylab=expression(paste(Delta, "APRE")), .xlab="Party Separation", expression("p=4"), .1))
legend("topleft", c("APRE2-APRE1", "APRE3-APRE2", "APRE4-APRE3"),  col=c("red", "blue", "darkgreen"), pch=c(19,17,7), lty=c(1,2,4), cex=.9, bty="n")
